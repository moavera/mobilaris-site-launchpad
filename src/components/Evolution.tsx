...
export const Evolution = () => {
  const highlights = [
    ...
    {
      icon: CheckCircle2,
      text: "Enhanced 3D visualization and real-time awareness",
    },
    {
      icon: ArrowRight,
      ...
      <div className="container relative z-10 mx-auto px-4 max-w-5xl">
        ...
        <div className="text-center mb-20 animate-fade-in-up">
          ...
          <p className="text-xl md:text-2xl text-foreground/70 max-w-3xl mx-auto leading-relaxed">
            Our platform builds on a long history of real-time positioning and 3D visualization in industrial and mining
            operations. The new Mobilaris 2.0 - Site™ is the next generation, redesigned for higher performance,
        ...
        <div className="grid md:grid-cols-3 gap-6 max-w-6xl mx-auto">
          {highlights.map((highlight, index) => (
            <div
              key={index}
              className="flex items-center gap-6 p-6 rounded-2xl bg-card/30 border border-border/40 backdrop-blur-sm hover:bg-card/50 hover:border-primary/20 transition-all duration-300 animate-fade-in-up"
              style={{
            ...
            >
              ...
              <p className="text-base md:text-lg text-foreground leading-relaxed">{highlight.text}</p>
            </div>
          ))}
        </div>
      </div>
